...
if df is None or df.empty:
    st.error(
        "Failed to load data. Please check if trip_kapal_listrik_data_rapi.csv exists in data folder."
    )
    st.stop()
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
    ["Data", "Charts", "KNN", "Export", "Info", "Model Analysis"]
)  # [web:19][web:7]
...
with tab1:
    ...
    try:
        if type_filter and day_filter:
            filtered_df = df[
                (df["type_trip"].isin(type_filter)) & (df["jenishari"].isin(day_filter))
            ].copy()
        else:
            filtered_df = df.copy()
    ...
    try:
        ...
        with col1:
            if "total_penumpang" in df.columns:
                ...
                ax.hist(
                    pd.to_numeric(df["total_penumpang"], errors="coerce"),
                    bins=15,
                    color="skyblue",
                    edgecolor="black",
                )
        ...
        with col1:
            if "jenishari" in df.columns:
                ...
                ax.bar(
                    day_counts.index, day_counts.values, color=["#95E1D3", "#F38181"]
                )
        ...
        with col2:
            if "persentaseisi" in df.columns:
                fig, ax = plt.subplots(figsize=(8, 5))
                ax.hist(
                    pd.to_numeric(df["persentaseisi"], errors="coerce"),
                    bins=20,
                    color="#A8D8EA",
                    edgecolor="black",
                )
    ...
    try:
        ...
        with col1:
            input_passengers = st.slider(
                ...
                min_value=1,
                max_value=int(
                    pd.to_numeric(df["total_penumpang"], errors="coerce").max()
                ),
                value=5,
            )
        ...
        with col2:
            if "type_trip" in df.columns:
                input_type = st.selectbox(
                    "Preferred Trip Type", df["type_trip"].unique()
                )
            else:
        ...
        with col3:
            if "jenishari" in df.columns:
                input_day = st.selectbox("Day Type", df["jenishari"].unique())
            else:
                input_day = "Weekday"
